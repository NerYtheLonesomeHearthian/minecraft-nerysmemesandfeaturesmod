...
public class StructureSakuratreeongrass extends ElementsNerYsFeaturesMod.ModElement {
	public StructureSakuratreeongrass(ElementsNerYsFeaturesMod instance) {
		super(instance, 53);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 20000) {
			...
			for (int a = 0; a < count; a++) {
				...
				IBlockState require;
				require = Blocks.TALLGRASS.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = Blocks.DOUBLE_PLANT.getStateFromMeta(2);
				try {
					if ((blockAt.getBlock() == require.getBlock())
				...
				require = Blocks.TALLGRASS.getStateFromMeta(1);
				try {
					if ((blockAt.getBlock() == require.getBlock())
							&& (blockAt.getBlock().getMetaFromState(blockAt) == require.getBlock().getMetaFromState(require)))
						blockCriteria = true;
				...
				require = Blocks.TALLGRASS.getStateFromMeta(0);
				try {
					if ((blockAt.getBlock() == require.getBlock())
							&& (blockAt.getBlock().getMetaFromState(blockAt) == require.getBlock().getMetaFromState(require)))
						blockCriteria = true;
				...
				require = Blocks.TALLGRASS.getStateFromMeta(2);
				try {
					if ((blockAt.getBlock() == require.getBlock())
							&& (blockAt.getBlock().getMetaFromState(blockAt) == require.getBlock().getMetaFromState(require)))
						blockCriteria = true;
