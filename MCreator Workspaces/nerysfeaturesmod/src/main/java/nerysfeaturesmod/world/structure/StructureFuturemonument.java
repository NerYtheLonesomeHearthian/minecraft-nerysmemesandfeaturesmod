...
import nerysfeaturesmod.block.BlockLichenblackstone;
...
public class StructureFuturemonument extends ElementsNerYsFeaturesMod.ModElement {
	public StructureFuturemonument(ElementsNerYsFeaturesMod instance) {
		super(instance, 318);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldTest.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldTest.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 5000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = BlockLichenblackstone.block.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("nerysfeaturesmod:nether1")))
					biomeCriteria = true;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("nerysfeaturesmod", "future_monument"));
				if (template == null)
					return;
